...
export default function StudentAnnouncements() {
    const q = useQuery({
        queryKey: ["announcements", "my", "student"],
        queryFn: () => AnnouncementsApi.my(),
    });

    return (
        <AppScreen title="Announcements" subtitle="School notices">
            ...
            {!q.isLoading && q.data?.length === 0 ? (
                ...
                <>
                    ...
                    {q.data?.map((a) => (
                        <ListRow
                            key={a._id}
                            left={<Text style={{ fontSize: 18, marginRight: 12 }}>{a.audienceType === 'school' ? '📢' : '📝'}</Text>}
                            title={a.title}
